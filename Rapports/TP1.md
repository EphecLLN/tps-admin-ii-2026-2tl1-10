...
Une fois cela fait il affiche le message "Hello from Docker!", sginfiant que l'image a été télchargé et fonctionne correctemment. 

Dans la console l'esemble des résultats est le suivant : 
**METTRE IMAGE Q1 ICI** 
...
3. Quel est son état ? 
Le status est : `Exited (0) 5 minute ago`, Autremment-dis l'image a été arrêté, car il a finis son exécution
...
En allant dans l'interface graphique de cette image (accessible en cliquant dans le nom de l'image de notre conteneur), on peut contrairement au terminal voir ce qui a été exectuée ainsi que la taille que cela prend. Ensuite tout comme le terminal on retrouve bien l'ID, ainsi que la taille. Enfin une autre info pouvant être uniquement observé sur l'interface graphique depuis combien de temps l'image est créer.  
...
Pour pouvoir supprimer une image sur le disque il faut utiliser la commande `docker rmi <id_image ou nom_de_l_image>`. En reprennant l'ID de notre image, on constate que par défaut la supression ne fonctionne pas. 
**METTRE IMAGE Q1.3.3 A**
La raison est que la suppresion doit être forcée, car elle est utilisée dans un container (celui que nous avons crées), il faut pour cela ajouter l'option -f qui force la supression. Et en ajouotant cette option, l'image est bien suprimée 
**METTRE IMAGE Q1.3.3 B** 
...
- L'option `i` permet de préciser au contenur de recevoir ce qui est tapé au clavier sans ceci le conteneur ne pourrait pas prendre en compte les commandes. 
- L'option `t` permet de faire en sorte que le texte s'affiche correctemment et de faire en sorte qu'il soit coprhensible pour un humain. Elle permet d'afficher le terminal ainsi que les lignes. 
...
Avec `docker inspect`, en cherchant dans les informations on trouve une section "NetworkSettings" avec "IPAddress", et on trouve que l'adresse IP est `172.17.0.1`
...
En testant les 2 commandes, nous avons crées pour les 2 situations le port 80 sur lequel on veut mettre le serveur en écoute. Mais la différence majeure est que pour la 1ère commande le port 80 n'est pas reliée, et en inspectant le conteneur, on repère bien le port 80 mais il affiche une valeur nulle, cela est du au fait que nous n'avons pas connecté le port 80 à un port localhost. 
...
Ensuite dans ce container on exécute le bash avec `docker exec -it mon-serveur-web bash`. A l'intérieur on met à jour les bibliothèques avec `apt-get update`, et ensuite on installe nano pour éditer le ficher HTML, ainsi que netstat pour vérifier que le port sont bien sur écoute avec `apt-get install -y nano net-tools`. 
...
J'ai d'abord tapé sur internet, image apache server, cela m'a ensuite redirigé vers dockerhub, où j'ai trouvé que le nom de l'image recherché est `httpd`.

![Site DockerHub avec l'image](/pictures/4.1%20image%20apache.png) 
...
![Localhost par défaut avec apache](/pictures/4.1%20localhost.png)
...
Port 80 : 
![Localhost port 80](/pictures/4.1%20localhost%20perso%20port80.png)

Port 8080 :
![Localhost port 8080](/pictures/4.1%20localhost%20perso%20port8080.png) 

#### Partie 4 : Réalisation de l'image personnalisée avec le DockerFile
On prépare un dossier nommée `apache-auto`, dedans on met un dossier site1 et site2 avec leur index.html repsectif pour chacunes des pages des 2 ports. A la racine on met le dockerfile ainsi que le ficher de config. 

On personalise d'abord le contenu des 2 fichiers HTML puis on configure le ficher de config nomée `vhosts.conf`. On y remet le même contenu comme fait dans le ficher de config précédemment. 

Une fois l'ensemble préparer on s'occupe du Dockerfile, on récupère d'abord l'image d'appache, on copie les 2 dossiers des sites à l'intérieur du conteneur, on y copie le fihcer personalisé, et on ajoute dans le ficher de config du contenur 2 lignes pour ouvrir le port 8080 et mettre le ficher de configuration personnalisé. Le Dockerfile est de cette forme : 
``` dockerfile
# 1. Image de base
FROM httpd:latest
...
Dans le dockerfile, on récupère directement l'image de python, on définit ensuite dans le conteneur un dossier de travail dans lequel va s'exécuter le script avec la commande `WORKDIR`. On copie ensuite le script python de la machine hôte vers le conteur, puis avec la commande `CMD`, on ordonne à notre conteneur d'exécuter le ficher `date.py` avec python. L'utilisation de cette commande permets aussi d'éteindre le conteneur dès que le script a été exécutée. 
A l'exécution on retrouve bien la date d'aujourd'hui avec l'heure d'exécution : 
![Script avec date et heure d'aujourd'hui](/pictures/4.3%20date.png)